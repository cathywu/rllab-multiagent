...
class SharedMLPPolicy(StochasticPolicy, LasagnePowered, Serializable):
    def __init__(
            self,
            name,
            env_spec,
        ...
        super(SharedMLPPolicy, self).__init__(env_spec)
    ...
    def get_actions(self, observations, env_infos=None):
        ...
        if self.training and env_infos is not None:
            ...
            flat_obs_n = [
                self.env_spec[i].observation_space.flatten_n(observations_n[i])
                for i in range(n)]
        dist_info_n = [dict(zip(['mean', 'log_std'], self.policies[0]._f_dist(flat_obs_n[i]))) for i in
                       range(n)]
        actions_n = [
